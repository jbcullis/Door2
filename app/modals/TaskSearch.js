...
module.exports = class TaskSearch extends Component {
    constructor(props) {
        super(props);
        this.Titlebar = null;
        this.state = {
    ...
    IsActive() {
        try {
            ...
            global.Log({Message: 'TaskSearch.IsActive>>' + ex.message});
        }
    };
    async Show() {
        try {
            global.root.NotificationModal.Show({ NotificationTitle: 'Loading...', NotificationStyle: 'Wait' });
            let _CalendarFilter = await Global.GetCalendarFilter();
            let _Params = {
                SearchType: 'task',
                SearchText: _CalendarFilter.SearchText,
                SearchStatus: _CalendarFilter.SearchStatus,
                SearchLimit: 25,
                SearchOffset: 0,
            };
            ...
            global.Log({Message: 'TaskSearch.Show>>' + ex.message, Notify: true});
        }
    };
    Hide() {
        try {
            ...
            global.Log({Message: 'TaskSearch.Show>>' + ex.message});
        }
    };
    ...
    async Search(SearchID_Value) {
        try {
            ...
            global.Log({Message: 'TaskSearch.Search>>' + ex.message, Notify: true});
        }
    };
    async Update(Ticket_Value) {
        try {
            let _Updated = false;
            for (let _TicketIndex = Global.State[this.props.ModelID].TicketList.length - 1; _TicketIndex >= 0; _TicketIndex--) {
                ...
                if (_Ticket.TicketID === Ticket_Value.TicketID) {
                    Global.State[this.props.ModelID].TicketList.splice(_TicketIndex, 1, TicketHelper.Clone(Ticket_Value));
                    _Updated = true;
                }
            }
            if (!_Updated) {
                Global.State[this.props.ModelID].TicketList.push(TicketHelper.Clone(Ticket_Value));
            }
            this.forceUpdate();
        } catch (ex) {
            global.Log({Message: 'TaskSearch.Update>>' + ex.message});
        }
    };
    async Delete(TicketID_Value) {
        try {
            ...
            global.Log({Message: 'TaskSearch.ClearFocus>>' + ex.message});
        }
    }
    ClearFocus() {
        try {
    ...
    render() {
        try {
                ...
                return (
                    <View style={{flex: 1, backgroundColor: Global.Theme.Header.BackgroundColor}}>
                        ...
                        <View style={[
                            ...
                            <View style={{flexDirection: 'row', height: 50}}>
                                ...
                                <Pressable style={{flex: 1, height: 50, justifyContent: 'center'}} onPress={() => this.ClearFocus()}>
                                    <Text style={{fontSize: 20, fontWeight: 'bold', color: Global.Theme.Header.ForegroundColor}}>TASKS ({Global.State[this.props.ModelID].TicketCount})</Text>
                                </Pressable>
                                <Pressable 
                                    onPress={() => {
                                        global.root.ShowTicket({
                                            TicketID: null,
                                            SaveCallback: (Ticket_Value) => this.Update(Ticket_Value),
                                            DeleteCallback: null,
                                        });
                                    }}
                                    style={({pressed}) => [{width: 50, height: 50, alignItems: 'center', justifyContent: 'center', opacity: pressed ? .5 : 1}]}
                        ...
                        refreshControl={
                            <RefreshControl refreshing={this.state.Refreshing} onRefresh={async () => {
                                try {
                                    ...
                                    global.Log({Message: 'TaskSearch.IsActive>>' + ex.message, Notify: true});
                                    this.setState({Refreshing: false});
                                }
                            }} />
                        }
                            ...
                            try {                        
                                if (Global.State[this.props.ModelID].LazyLoadEnabled) {
                                    ...
                                    TicketHelper.GetList({
                                        ...
                                        global.Log({Message: 'TaskSearch.render>>' + ex.message, Notify: true});
                                    });
                                }
                            } catch (ex) {
                                global.Log({Message: 'TaskSearch.render>>' + ex.message, Notify: true});
                        ...
                        renderItem={({item, index, separators}) => {
                            ...
                            if (_Ticket.TicketCreated !== null) {
                                let _CreatedDate = new Date(_Ticket.TicketCreated + 'Z');
                                _CreatedDescription = Global.TimeSince(_CreatedDate);
                            }
                            ...
                            let _TicketUI = (
                                <Pressable 
                                    ...
                                    onPress={() => {
                                        global.root.ShowTicket({
                                            ...
                                            SaveCallback: (Ticket_Value) => this.Update(Ticket_Value),
                                            DeleteCallback: (TicketID_Value) => this.Delete(TicketID_Value),
                                        });
                                    }}
                                ...
                                >
                                    ...
                                    <View style={{flex: 1, justifyContent: 'center', padding: 10}}>
                                        <Text style={{color: Global.Theme.Body.ForegroundFade, fontSize: 11}}>{_CreatedDescription}</Text>
                                        <Text style={{marginTop: 4, color: Global.Theme.Body.ForegroundColor}}>{_Ticket.TicketDescription}</Text>
                                    </View>
                        ...
                        ListEmptyComponent={() => {
                            return (
                                <View key={'No_Tasks_Found'} style={{flex: 1, height: 50, margin: 10, backgroundColor: Global.Theme.Body.ControlBackground, borderRadius: 4, alignItems: 'center', justifyContent: 'center'}}>
                                    <Text style={{color: Global.Theme.Body.ForegroundFade}}>No Tasks Found</Text>
                                </View>
                            );
                        }} />
            ...
            global.Log({Message: 'TaskSearch.render>>' + ex.message});
        }
    };
};