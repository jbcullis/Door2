...
module.exports = class EventSearch extends Component {
    constructor(props) {
        super(props);
        this.Titlebar = null;
        this.state = {
    ...
    IsActive() {
        try {
            ...
            global.Log({Message: 'EventSearch.IsActive>>' + ex.message});
        }
    };
    async Show() {
        try {
            ...
            global.Log({Message: 'EventSearch.Show>>' + ex.message, Notify: true});
        }
    };
    Hide() {
        try {
            ...
            global.Log({Message: 'EventSearch.Show>>' + ex.message});
        }
    };
    ...
    async Search(SearchID_Value) {
        try {
            ...
            global.Log({Message: 'EventSearch.Search>>' + ex.message, Notify: true});
        }
    };
    async Update(Transaction_Value) {
        try {
            ...
            global.Log({Message: 'EventSearch.Update>>' + ex.message});
        }
    };
    async Delete(Transaction_Value) {
        try {
            ...
            global.Log({Message: 'EventSearch.Delete>>' + ex.message});
        }
    };
    ClearFocus() {
        try {
            ...
            global.Log({Message: 'EventSearch.ClearFocus>>' + ex.message});
        }
    };
    ...
    ActiveHandler() {
        try {
            ...
            global.Log({Message: 'EventSearch.ActiveHandler>>' + ex.message});
        }
    };
    BackHandler() {
        try {
            ...
            global.Log({Message: 'EventSearch.BackHandler>>' + ex.message});
        }
    };
    KeyboardHandler(action, keyboardheight) {
        try {
            ...
            global.Log({Message: 'EventSearch.KeyboardHandler>>' + ex.message});
        }
    };
    ...
    render() {
        try {
                ...
                return (
                    <View style={{flex: 1, backgroundColor: Global.Theme.Header.BackgroundColor}}>
                        ...
                        <View style={[
                            ...
                            <View style={{flexDirection: 'row', height: 50}}>
                                ...
                                <View style={{flex: 1, height: 50, alignItems: 'center', justifyContent: 'center', marginRight: 50}}>
                                    <Text style={{fontSize: 20, fontWeight: 'bold', color: (global.ColorScheme === 'dark' ? '#eeeeee' : '#121212')}} numberOfLines={1}>Events</Text>
                                </View>
                            </View>
                        ...
                        <FlatList 
                            ...
                            refreshControl={
                                <RefreshControl refreshing={this.state.Refreshing} onRefresh={async () => {
                                    try {
                                        ...
                                        global.Log({Message: 'EventSearch.IsActive>>' + ex.message, Notify: true});
                                        this.setState({Refreshing: false});
                                    }
                                }} />
                            }
                                ...
                                try {                        
                                    if (Global.State[this.props.ModelID].LazyLoadEnabled) {
                                        ...
                                        TransactionHelper.GetList({
                                            ...
                                            global.Log({Message: 'EventSearch.render>>' + ex.message, Notify: true});
                                        });
                                    }
                                } catch (ex) {
                                    global.Log({Message: 'EventSearch.render>>' + ex.message, Notify: true});
                            ...
                            ListEmptyComponent={() => {
                                return (
                                    <View key={'No_Events_Found'} style={{flex: 1, height: 50, margin: 10, backgroundColor: Global.Theme.Body.ControlBackground, borderRadius: 4, alignItems: 'center', justifyContent: 'center'}}>
                                    <Text style={{color: Global.Theme.Body.ForegroundFade}}>No Upcoming Events</Text>
                                </View>
                                );
                            }}
                        />
            ...
            global.Log({Message: 'EventSearch.render>>' + ex.message});
        }
    };
};